...
function title_set_menu(id) {
  ...
  if (id == TITLE_MENU_MAIN) {
    ...
    title.menu[0] = "Based on Heroine Dusk" ;
	title.menu[1] = "by Clint Bellanger";
    title.menu[2] = "Mods - Jefferson Carpenter";
	title.menu[3] = "& SpacePirate Games"; 
	title.menu[4] = "Secret #0 A-OL186";
	title.menu[5] = "Back";
...
function title_render() {
  ...
  for (var i=0; i<title.menu.length; i++) {
    if (title.menu_selector == i) {
      bitfont_render("[ "+title.menu[i]+" ]", 80, 55+(i*title.text_h), JUSTIFY_CENTER);
    }
    else {
      bitfont_render(title.menu[i], 80, 55+(i*title.text_h), JUSTIFY_CENTER);
    }
